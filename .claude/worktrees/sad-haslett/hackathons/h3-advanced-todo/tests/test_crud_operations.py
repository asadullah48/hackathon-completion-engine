...
# Test client - uses database override from conftest
...
class TestHealthCheck:
    ...
    def test_root_endpoint(self):
        ...
        assert response.status_code == 200
        data = response.json()
        assert data["name"] == "H3 Advanced Todo API"
        assert "endpoints" in data
        assert "recurring" in data["endpoints"]
        assert "templates" in data["endpoints"]
