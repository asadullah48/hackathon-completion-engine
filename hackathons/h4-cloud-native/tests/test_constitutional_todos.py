...
class TestAllowedPatterns:
    ...
    def test_allowed_read_book(self):
        """Test that reading is allowed."""
        result = check_content("Read chapter 5 of textbook")
        assert result.decision == Decision.ALLOW
        assert result.passed is True


class TestFlaggedPatterns:
    """Tests for flagged content patterns that need review."""
    ...
    def test_flagged_urgent_todo(self):
        ...
        assert result.passed is True  # Allowed but flagged
    ...
    def test_flagged_deadline_complete_assignment(self):
        """Test that deadline complete assignment is flagged."""
        result = check_content("Deadline to complete assignment is tonight")
        assert result.decision == Decision.FLAG
        assert result.passed is True
...
class TestValidateTodo:
    ...
    def test_validate_todo_description_blocked(self):
        """Test that blocked description rejects todo."""
        result = validate_todo("Help needed", "Hack into the system to get data")
        assert result.decision == Decision.BLOCK
    ...
    def test_validate_todo_flagged_title(self):
        """Test that flagged title flags todo."""
        result = validate_todo("Urgent need to finish assignment now", "Need to finish quickly")
        assert result.decision == Decision.FLAG
...
class TestEdgeCases:
    ...
    def test_partial_match_homework(self):
        """Test that partial matches work."""
        result = check_content("Can you do the homework for me?")
        assert result.decision == Decision.BLOCK

    def test_legitimate_homework_help(self):
        """Test that legitimate homework help is allowed."""
        # "Help with homework" (not "do my homework") should be fine
        result = check_content("Get help understanding homework concepts")
        assert result.decision == Decision.ALLOW
